estimateCME_PLR <- function(
  ...
  if (need_tune) {
    ...
    if (length(selected_union) > 0) {
      if (verbose) cat("  → Union of selected covariates has", length(selected_union), "columns.  Refitting OLS...\n")
      # Refit outcome on union
      X_sub_outcome <- XZ_design[, selected_union, drop = FALSE]
      df_out_sub    <- data.frame(y = Yvec, X_sub_outcome)
      final_out_lm  <- lm(y ~ ., data = df_out_sub)
      outcome_fit   <- list(fit = final_out_lm, type = "lm")
      
      # Refit treatment on union
      X_sub_treat   <- XZ_design[, selected_union, drop = FALSE]
...
bootstrapCME_PLR <- function(
  ...
  coverage     <- uni_res$coverage # scalar coverage
