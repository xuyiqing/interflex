...
if(TRUE){## bandwidth selection
  ...
  #print(bw)
  if(predict==TRUE){ #predict functions
  ...
  if(is.null(diff.values)==FALSE){
	...
	if(treat.type=='discrete'){
		gen_difference <- function(data,diff.values,coef){
			...
			#coef <- coefs.new(data=data,D=D,Y=Y,Z=Z,X=X,bw=bw,FE=FE,base = base,weights = weights,predict=FALSE,
            #          treat.type = 'discrete',X.eval = X.eval,bw.adaptive = bw.adaptive)
