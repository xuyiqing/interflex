estimateCME_PLR <- function(data,
  ...
  #}
  if (need_tune && verbose) {
    message("Either outcome_model_type and treatment_model_type needs to be tuned': ",
  ...
  if(outcome_model_type=='linear'){
    outcome_fit <- do_single_fit(Yvec, XZ_design, outcome_model_type, NULL)
  }   
  else{
      if(is.null(lambda_cv)){ #cross-validation
        ...
        lambda_outcome <- lambda_cv[['outcome']]
        outcome_fit <- do_single_fit(Yvec, XZ_design, outcome_model_type, lambda_outcome)
      }
  }

  if(treatment_model_type=='linear'){
    treatment_fit <- do_single_fit(Dvec, XZ_design, treatment_model_type, NULL)
  }   
  else{
      if(is.null(lambda_cv)){ #cross-validation
        treatment_fit <- do_single_fit(Dvec, XZ_design, treatment_model_type, NULL)
        ...
        treatment_fit <- do_single_fit(Dvec, XZ_design, treatment_model_type, lambda_treatment)
      }
  }
  ...
  if (need_tune) {
    ...
    lambda_cv_save <- list(outcome = outcome_fit$lambda, treatment = treatment_fit$lambda)
    # Gather active sets
...
bootstrapCME_PLR <- function(
  ...
  need_tune   <- !is.null(selected_covars)  # if union_covars is non-NULL => both-lasso
