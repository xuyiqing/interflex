...
interflex <- function(estimator, # "linear", "kernel", "binning" , "gam", "raw", "DML"
    ...
    if (estimator == "DML") {
        output <- interflex.DML(
            ...
            CV.final = CV.final,
            param.grid.final = param.grid.final,
            n.folds.final = n.folds.final,
            scoring.final = scoring.final,
            featurizer.model.final = featurizer.model.final,
            featurizer.param.final = featurizer.param.final,
            n.jobs = n.jobs,
            treat.info = treat.info,
            diff.info = diff.info,
            figure = figure,
