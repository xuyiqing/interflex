##Input Check
interflex <- function(estimator, #"linear", "kernel", "binning" , "gam", "raw"
								data,
								Y, # outcome
...
##################################################### CHECK ##############################################################
  ...
  ## estimator
  if(!estimator%in%c("linear","binning","kernel","gam","raw")){
	stop("estimator must be one of the following: raw, linear, binning, kernel or gam.\n")
  }
...
##################################################### Preprocess ##############################################################
	...
	if(estimator=="linear"){
		output <- interflex.linear(data=data,
									Y=Y, # outcome
	...
	if(estimator=="binning"){
		output <- interflex.binning(data = data,
									  Y = Y, # outcome
									  D = D, # treatment indicator
									  X = X, # moderator
									  treat.info = treat.info,
	...
	if(estimator=="kernel"){
		output <- interflex.kernel(data=data,
									Y=Y, # outcome
									D=D, # treatment indicator
									X=X, # moderator
									treat.info=treat.info,
	...
	if(estimator=='gam'){
		...
		output <- interflex.gam(data = data,
                    		   Y = Y,
                    		   D = D,
                    		   X = X,
                    		   Z = Z,
	...
	if(estimator=='raw'){
		output <- interflex.raw(data = data,
                       		   Y = Y,
                       		   D = D,
                       		   X = X,
                       		   treat.info = treat.info,
