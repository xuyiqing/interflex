...
To use double/debiased machine learning (DML) estimators, please install the **interflex** package directly from Github:
```{r eval=FALSE}
...
The DML estimator in **interflex** now uses pure R packages — **no Python installation is required**. The following R packages are used internally and will be installed automatically as dependencies:

- **DoubleML**: Core DML framework (R6 classes for IRM and PLR models)
- **mlr3** + **mlr3learners**: Machine learning model backends
- **ranger**: Random forest (default ML method)
- **data.table**: Required by DoubleML internals
- **paradox**: Tuning parameter specifications (used with cross-validation)

For non-default ML methods, you may also need:

- **lightgbm**: For histogram-based gradient boosting (`model.y = "hgb"` or `model.t = "hgb"`)
- **nnet**: For neural networks (`model.y = "nn"` or `model.t = "nn"`)
- **mlr3tuning**: For cross-validation tuning (`CV = TRUE`)

These optional packages can be installed as needed:
```{r eval=FALSE}
install.packages(c("lightgbm", "nnet", "mlr3tuning"))
```

### Load packages

These are packages necessary for this tutorial:
...
Users can customize parameters for the machine learning model in both stages by specifying the options `param.y` and `param.t`. The DML estimator accepts parameter names in both sklearn style (for backward compatibility) and native R package style. Internally, sklearn-style names are mapped to their R equivalents:

- **Random forest** (`rf`): Uses the `ranger` package. Key parameters: `num.trees` (or `n_estimators`), `max.depth` (or `max_depth`), `min.node.size` (or `min_samples_leaf`), `mtry` (or `max_features`).
- **Gradient boosting** (`hgb`): Uses the `lightgbm` package. Key parameters: `num_leaves` (or `max_leaf_nodes`), `min_data_in_leaf` (or `min_samples_leaf`), `lambda_l2` (or `l2_regularization`).
- **Neural network** (`nn`): Uses the `nnet` package. Key parameters: `size` (or `hidden_layer_sizes`), `maxit` (or `max_iter`), `decay` (or `alpha`).

Here we use dataset **s7** to explore how selecting different DML parameters affects performance. We can directly specify the size of the neural networks, the activation function, learning rate, and solver using the list `param_nn` and feed them to the DML estimator.
