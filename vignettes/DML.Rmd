...
Users can also change the machine learning method for Stage 1. Now `Random Forest` (rf), `Hist Gradient Boosting` (hgb), and `Neural Network` (nn) are allowed to input as `ml_method`.
...
For continuous treatment specifically, users can also change the different methods for double machine learning itself. Now `Default`, `Polynomial`, `Regularization` (remember to control the `lasso_alpha` parameter to control the penalty), and `Non-Parametric` are allowed to input as `dml_method`.

```{r dlinear6, results = "hide", fig.height=5, fig.width=8}
s2.DML.hgb <- interflex(estimator='DML', data = s7, ml_method="hgb",
                        Y = "Y", D = "D", X = "X", Z = c("Z1", "Z2"),
...
s7.DML.rf <- interflex(estimator='DML', data = s7, ml_method="rf",
                        Y = "Y", D = "D", X = "X", Z = c("Z1", "Z2"),
                        D.ref = c(0.25,0.5,0.75),
                        dml_method = "regularization", lasso_alpha = 0.00001,
                        treat.type = "continuous")
plot(s7.DML.rf)

s7.DML.hgb <- interflex(estimator='DML', data = s7, ml_method="hgb",
                        Y = "Y", D = "D", X = "X", Z = c("Z1", "Z2"),
                        D.ref = c(0.25,0.5,0.75),
...
```{r dlinear10, results = "hide", fig.height=5, fig.width=8}
